...
class Zero123(nn.Module):
    ...
    def train_step(self, embeddings, pred_rgb, polar, azimuth, radius, guidance_scale=3, as_latent=False, grad_scale=1, save_guidance_path:Path=None):
        # embeddings = {'zero123_ws': [1, 1], 'ref_polars': [90, 90], 'ref_azimuths': [0, 180], 'ref_radii': [3, 3]}
        # pred_rgb = tensor(torch.Size([1, 3, 64, 64])), 
        # polar = tensor([-27.7486], device='cuda:0')
        # azimuth = tensor([127.7527], device='cuda:0')
        # radius = tensor([0.1511], device='cuda:0')
        # guidance_scale = 5
        # as_latent = False
        # grad_scale = 1
        ...
        # v1 = tensor([[3.1511, 1.0865, 2.2297]], device='cuda:0')
        ...
        # v2 = tensor([[3.0000, 1.5708, 0.0000],
        # [3.0000, 1.5708, 3.1416]])
        angles = torch.rad2deg(self.angle_between(v1, v2)).to(self.device)
        # angles = tensor([[122.8093,  57.1907]], device='cuda:0')
        ...
        if as_latent:
            ...
            # encode img to latents: torch.Size([1, 4, 32, 32])
        ...
        if len(ref_azimuths) > 1:
            inv_angles = 1/angles #smaller angle -> bigger inv_angle
            inv_angles[inv_angles > 100] = 100
            inv_angles /= inv_angles.max(dim=-1, keepdim=True)[0] # normalization
            inv_angles[inv_angles < 0.1] = 0
            # tensor([[0.4657, 1.0000]], device='cuda:0')
        else:
            inv_angles = torch.tensor([1.]).to(self.device)

        # Multiply closeness-weight(inv_angles) by user-given weights
        zero123_ws = torch.tensor(embeddings['zero123_ws'])[None, :].to(self.device) * inv_angles
        zero123_ws /= zero123_ws.max(dim=-1, keepdim=True)[0] # normalization
        ...
        with torch.no_grad():
            ...
            for (zero123_w, c_crossattn, c_concat, ref_polar, ref_azimuth, ref_radius) in zip(zero123_ws.T,
                ...
                noise_pred = self.model.apply_model(x_in, t_in, cond)# denoise: predict noise
                noise_pred_uncond, noise_pred_cond = noise_pred.chunk(2)
                noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)# classifier-free guidance
                noise_preds.append(zero123_w[:, None, None, None] * noise_pred)# zero123_w is closeness-weight(inv_angles) * user-given weight

        noise_pred = torch.stack(noise_preds).sum(dim=0) / zero123_ws.sum(dim=-1)[:, None, None, None] # normalization
        ...
        grad = (grad_scale * w)[:, None, None, None] * (noise_pred - noise) #compute gradient
        ...
        loss = 0.5 * F.mse_loss(latents.float(), targets, reduction='sum') / latents.shape[0] #small grad -> small loss
